...
module.exports = (bot, flow) =>
  createFeature(bot, flow, {
    key: "compat",
    buttonId: "compat_start",
    label: "совместимость пары",
    price: 50,
    waitText: "💞 Сверяю звёздные паспорта…",
    hintText:
      "❗ Две карты, две строки (можно с «1)» «2)»):\n" +
      "1) Дата ⏰ Время 🗺 Город\n" +
      "2) Дата ⏰ Время 🗺 Город",
    askText:
      "✨ Оплата подтверждена!\n\n" +
    ...
    sysMsg:
      "Отвечай *ровно пятью* блоками (1-5), каждый ≤3 строки. Без лишних заголовков.",
    buildPrompt: ([, cardA, cardB]) =>
      `
Сделай совместимость пары по 5 блокам:
1. 🌟 Общее впечатление
2. 💗 Эмоции / быт
3. 🔥 Интим / страсть
4. 🤝 Конфликты и рост
...
Партнёр A: ${cardA.trim()}
Партнёр B: ${cardB.trim()}`.trim(),
  });
