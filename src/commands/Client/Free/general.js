...
// РЕГУЛЯРКА для одной натальной карты (натальной)
...
Дата рождения: ${ctx.message.text.trim()}`;
      ...
      await ctx.reply(
        "🛠️ Космос молчит. Попробуй позже.\n\n💎 *Хочешь узнать о любви, деньгам или совместимости?* Нажми соответствующую платную кнопку!",
        { parse_mode: "Markdown" }
      );
      flow.delete(ctx.from.id);
    }
  ...
  /* ── Если пользователь в режиме “general” написал что-то невалидное ── */
  bot.on("message", async (ctx) => {
    if (flow.get(ctx.from.id) === "general") {
      await ctx.reply(
        "❗ Формат неверный. Отправь *дату, время и город* в формате:\n" +
          "`ДД.MM.ГГГГ ЧЧ:ММ Город` (например, `01.01.2000 10:00 Москва`)",
        { parse_mode: "Markdown" }
      );
      flow.delete(ctx.from.id);
    }
